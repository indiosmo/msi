...
std::pair<bool, uint32_t>
low::entry_time() const
{
  return std::pair<bool, uint32_t>( presence_map_[ix_entry_time], entry_time_ );
}

void
low::set_entry_time( uint32_t val )
{
  entry_time_ = val;
  presence_map_[ix_entry_time] = true;
}

const std::bitset<low::ix_field_count>&
low::presence_map() const
{
  return presence_map_;
}

void low::set_presence_map( uint32_t val )
{
