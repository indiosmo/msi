...
open_serializer::open_serializer()
{
  data_.resize( 500 +
...
open_serializer::serialize(
  const messages::open& msg )
{
...
std::size_t
open_serializer::buffer_size() const
{
  return data_.size();
}

std::size_t
open_serializer::deserialize( const unsigned char* data,
                              messages::open& dst )
{
  decimal d;
  std::size_t offset = offset_variable_fields;
  ...
  if ( dst.presence_map()[messages::open::ix_settl_flag] )
  {
