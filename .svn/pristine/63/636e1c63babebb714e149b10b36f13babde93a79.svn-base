#ifndef APOENA_MSI_MARKETDATA_SERIALIZATION_BID_SERIALIZER_HPP
#define APOENA_MSI_MARKETDATA_SERIALIZATION_BID_SERIALIZER_HPP
...
class MARKETDATA_SERIALIZATION_Export bid_serializer
{
public:
  enum offset
  {
    ...
    offset_buyer = offset_entry_time + sizeof( uint32_t ),
    offset_position = offset_buyer + sizeof( uint32_t ),
    offset_order_id = offset_position + sizeof( uint32_t ),
    offset_variable_fields = offset_order_id + sizeof( uint32_t )
  };

  bid_serializer();
  std::pair<const unsigned char*, std::size_t> serialize( const messages::bid& msg );
  std::size_t buffer_size() const;
  std::size_t deserialize( const unsigned char* data, messages::bid& dst );
...
#endif /* APOENA_MSI_MARKETDATA_SERIALIZATION_BID_SERIALIZER_HPP */
