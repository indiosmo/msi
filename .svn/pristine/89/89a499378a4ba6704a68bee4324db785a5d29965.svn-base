...
low_serializer::serialize(
  const messages::low& msg )
{
...
std::size_t
low_serializer::buffer_size() const
{
  return data_.size();
}

std::size_t
low_serializer::deserialize( const unsigned char* data,
                             messages::low& dst )
{
  decimal d;
  std::size_t offset = offset_variable_fields;
