...
vwap_serializer::vwap_serializer()
{
  data_.resize( 500 +
...
vwap_serializer::serialize(
  const messages::vwap& msg )
{
...
std::size_t
vwap_serializer::buffer_size() const
{
  return data_.size();
}

std::size_t
vwap_serializer::deserialize( const unsigned char* data,
                              messages::vwap& dst )
{
  decimal d;
  std::size_t offset = offset_variable_fields;
