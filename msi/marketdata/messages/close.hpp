#ifndef APOENA_MSI_MARKETDATA_MESSAGES_CLOSE_HPP
#define APOENA_MSI_MARKETDATA_MESSAGES_CLOSE_HPP
...
class MARKETDATA_MESSAGES_Export close
{
public:
  enum field_index
  {
  ...
  std::pair<bool, uint32_t> last_trade_date() const;
  void set_last_trade_date( uint32_t val );

  std::pair<bool, uint32_t> price_adjust_method() const;
  void set_price_adjust_method( uint32_t val );
...
private:
  ...
  uint32_t last_trade_date_;
  uint32_t price_adjust_method_;
...
#endif /* APOENA_MSI_MARKETDATA_MESSAGES_CLOSE_HPP */
